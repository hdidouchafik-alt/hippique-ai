...
app = FastAPI(title="Hippique AI", version="0.5.0")
...
class MultiTaskRequest(BaseModel): task: str = Field(min_length=1, max_length=4000); horse: str | None = None; sources: list[str] = Field(default_factory=list)
class PredictionRequest(BaseModel): race_key: str = Field(min_length=1, max_length=150); race_name: str = Field(default="Course importée", max_length=200); race_date: str | None = None; text: str = Field(min_length=20, max_length=100000)
class OutcomeRequest(BaseModel): arrival: list[int] = Field(min_length=1, max_length=50)
...
async def prediction_history(limit: int = 20): return {"predictions": prediction_store.history(limit)}
@app.get("/api/learning/metrics")
async def learning_metrics(): return prediction_store.metrics()
